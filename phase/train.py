...
from phase.preprocess import AugmentedPennActionDataset
...
if __name__ == "__main__":
    ...
    # load dataset
    dataset = AugmentedPennActionDataset(annotation_dir, stride=1)
    ...
    train_val_labels = [dataset.file_labels[i] for i in train_file_idx]
    train_file_idx, val_file_idx = train_test_split(
        train_file_idx,
        test_size=0.176,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_x, batch_phase, batch_action in val_loader:
            ...
            batch_action = batch_action.to(device)
            batch_phase = batch_phase.to(device)
            batch_phase = batch_phase[:, ::2]  # downsample phase labels to match model output (every 4 frames)
        ...
        val_loss = running_loss / total_action 
        print(f"Epoch {epoch+1}/{num_epochs} | Loss: {epoch_loss:.4f} | Accuracy: {acc:.4f} | MAE: {mae:.4f}  | Val_Loss: {val_loss:.4f}")
        ...
        if val_loss < best_val_loss - min_delta:
            ...
            counter = 0
            torch.save(model.state_dict(), "phase/best_model_phase.pth")
            print("Model saved.")
        else:
            counter += 1
    ...
    model.load_state_dict(torch.load("phase/best_model_phase.pth"))
    model.eval()
