...
model.to(device)
model.load_state_dict(torch.load("stgcn_model4.pth", map_location=device))
model.eval()
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if len(joint_buffer) == T:
                    ...
                    if pred_action == "pushup":
                        deep_enough, back_form = form_analysis(pred_action, prev_phase, x, visible_side)
                        deepnes += deep_enough
                        if back_form > 0:
                            rounded_back += 1
                        elif back_form < 0:
                        ...
                        print(deepnes)
                    if pred_action == "squat":
                        deep_enough, forward_bend = form_analysis(pred_action, prev_phase, x, visible_side)
                        deepnes += deep_enough
                        if forward_bend == 0:
                            perfect_form += 1
                        else:
